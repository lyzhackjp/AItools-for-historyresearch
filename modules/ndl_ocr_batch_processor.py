...
class NDLOCRBatchProcessor:
    ...
    def __init__(self, ndlocr_path: Optional[str] = None):
        ...
        self.results = []
        self._validate_ndlocr()
    
    def _validate_ndlocr(self) -> bool:
        """
        验证NDL OCR是否可用
    ...
    def process_batch(self, image_dir: str, output_dir: str,
        ...
        print(f"  平均: {elapsed/len(results):.1f}秒/页")
...
def create_batch_processor(ndlocr_path: Optional[str] = None) -> NDLOCRBatchProcessor:
    """
    工厂函数 - 创建批量处理器
    ...
    """
    return NDLOCRBatchProcessor(ndlocr_path)
