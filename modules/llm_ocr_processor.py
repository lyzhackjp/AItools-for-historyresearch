...
class QwenVLOCRProcessor:
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fallback_order": ["llm_api:dashscope", "unified_ocr_processor", "ocr_processor:tesseract"],
        }

    def build_pages_package(
        self,
        pages: List[ProcessedPage],
        source_path: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Wrap processed OCR pages in the shared workflow envelope."""
        ...
        if any(page.text_length == 0 for page in pages):
            quality_flags.append("empty_page_text")
        pages_with_ocr_number = sum(1 for page in pages if page.ocr_page_number is not None)
        if pages and pages_with_ocr_number == 0:
            quality_flags.append("no_ocr_page_numbers")
        confidence = 0.78 if full_text.strip() else 0.2
        if quality_flags:
        ...
        return {
            ...
            "statistics": {
                ...
                "total_characters": sum(page.text_length for page in pages),
                "pages_with_ocr_number": pages_with_ocr_number,
                "header_changes": sum(1 for page in pages if page.header_is_new),
                "footer_changes": sum(1 for page in pages if page.footer_is_new),
            },
            "artifacts": [],
            ...
            "metadata": self.get_capabilities(),
    ...
    def run_package(self, start_page: int, end_page: int) -> Dict[str, Any]:
        """Run the full LLM OCR workflow and return a workflow package."""
        result = self.run(start_page=start_page, end_page=end_page)
        package = self.build_pages_package(result.get("pages", []), source_path=self.pdf_path)
        package["statistics"].update(result.get("statistics", {}))
        return package
