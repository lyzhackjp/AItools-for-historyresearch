    ...
    def create_output_document(self, content: dict, output_path: str):
        ...
        try:
            doc_simple = {
                ...
                'paragraphs': content.get('paragraphs', [])[:20],
                'tables': [],
                'headers': [],
                'footers': []
            }

            success = self.doc_processor.create_document(doc_simple, output_path)
...
def main():
    ...
    if output_file.exists():
        ...
        print("✗ 输出文档创建失败")
        for step in workflow.processing_log['steps']:
            if step['step_name'] == '创建输出文档' and 'error' in step:
