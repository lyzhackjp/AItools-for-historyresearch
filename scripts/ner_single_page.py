"""
NER processing - Single page version with checkpointing
Processes pages 221-230 one at a time to avoid memory/timeout issues
"""
import json
import os
import sys
...
def parse_response(content):
    # Extract JSON
    json_match = re.search(r'\{[^{}]*"person_info"[^{}]*\}', content, re.DOTALL)
    if json_match:
        try:
...
def extract_person_entries(block_texts):
    """Extract individual person entries from combined block texts."""
    # Join all blocks
    full_text = '\n'.join(block_texts)

    # Split by name pattern (2-4 kanji at start of line)
...
def process_page(page_num):
    ...
    full_len = sum(len(t) for t in blocks)
    print(f"  Loaded {len(blocks)} blocks, {full_len} chars")

    # Split into person entries
...
def main():
    print("=" * 50)
    print("NER Processing - Pages 221-230")
    print("=" * 50)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    all_results = []

    # Process pages 221-230
